 ...
 * @return none
 ***********************************************/
void FFT2N128(int16_t* xwsk) //algorytm 2N FFT 1024 probki wejsciowe, rzeczywiste przeksztalcenie z wykorzystaniem zespolonego FFT
{
	//2N FFT
	...
	for (i1 = 0; i1 < 7; i1++) //7 razy dla Np/8=128, log2(128)=7
	{
		...
		uint16_t r = 0;
		for (i2 = 0; i2 < grp; i2++) {
			sinp = sinus(r / 4) >> 7; //r*512/128 - 512 elementow tablicy sin dla okresu, 128p. FFT
			cosp = cosinus(r / 4) >> 7; //+128 dla cos
	...
	*xwsk = log2_u32(xp >> 6);
	...
	for (i1 = 1; i1 < (Np / 16) + 1; i1++) {
		...
		xp = (int32_t) ywsk[Np / 8 - i1] * (int32_t) ywsk[Np / 8 - i1]
				+ (int32_t) xwsk[Np / 8 - i1] * (int32_t) xwsk[Np / 8 - i1];
		//syngal wyjsciowy jako logarytm modulu widma
